...
### EXECUTIVE SUMMARY
The simulation indicates a high probability that public sentiment toward the Federal Reserve will sour significantly over the next thirty days.`
      }];
    }
    ...
    if (path === "/api/runs/" && method === "POST") {
      ...
      __mockExpertMode = body?.expert_mode || false;
    ...
    if (path.includes("/report")) {
      return { report: __mockExpertMode ? mock.MOCK_EXPERT_REPORT : mock.MOCK_BASIC_REPORT };
    }
    if (path.includes("/cancel")) {
      return { ok: true };
    }
    if (path.includes("/api/runs/") && method === "GET") {
      return { status: __mockStatus, run_id: "demo-run-1", config: { n_agents: 5, n_rounds: 5, expert_mode: __mockExpertMode } };
    }
